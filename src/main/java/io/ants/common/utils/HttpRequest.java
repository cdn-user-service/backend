...
public class HttpRequest {
    ...
    public static  String erkHttpS(String uri,String password,String crtPath,String queryData){
        String ret="";
        // Load Certificate
        // curl --cacert /etc/elasticsearch/certstp_ca.crt -k -u elastic:Omg3GxT=MGMkkWLa-+sd https://localhost:9200/sample/_search -d'{  "query": {    "match": {"decode_data.domain": "test.com"}  }}'
        StringBuilder result=new StringBuilder();
    ...
    public static String httpGetFromCmd(String cmdStr){
        final Pattern URL_PATTERN =     Pattern.compile("((https?|ftp|gopher|telnet|file|notes|ms-help):((//)|(\\\\))+[\\w\\d:#@%/;$()~_?\\+-=\\\\\\.&]*)", Pattern.CASE_INSENSITIVE);

        // String text = "这是一个测试文本，其中包含了一个URL：https://www.example.com/path/to/page 和另一个 ftp://ftp.example.com/file.txt";
