...
public class IPSeeker {
    ...
    //private static final String IP_FILE = "C:\\Users\\Administrator\\Desktop\\qqwry.dat";
    // private static  String IP_FILE=QQWryPathBean.ip_file_path;
    private static File IP_FILE;
    private static File inuModel;
    static {
        Resource resource = null;
        InputStream inputStream = null;
        OutputStream outputStream = null;
        try {
            resource = new ClassPathResource("qqwry.dat");
            File inuModel = new File("qqwry.dat");
            inputStream = resource.getInputStream();
            outputStream = new FileOutputStream(inuModel);
            ...
            if (resource != null) {
                // 可能需要调用 resource.close() 或其他相应的方法来释放资源
            }
        }
    }
    ...
    public IPSeeker() {
        ...
        try {
            ...
            //System.out.println(IP_FILE.getPath());
    ...
    public String getAddress(String ip) {
        String country = " CZ88.NET".equals(getCountry(ip)) ? "null"
                : getCountry(ip);
        String area = " CZ88.NET".equals(getArea(ip)) ? "null" : getArea(ip);
        String address = country + "|" + area;
        return address.trim();
    }

    public static String getIpAreaByNew(String ip){
        String area="";
        try{
            if (IPUtils.isValidIPV4(ip)){
                IPSeeker is=new IPSeeker();
                area=is.getAddress(ip);
                is.ipFile.close();
            }
        }catch (Exception e){
            e.printStackTrace();
    ...
    public static synchronized  String getIpArea(String ip){
        IPSeeker is;
        try{
            is =  IPSeeker.getInstance();
            return is.getAddress(ip);
        }catch (Exception e){
            e.printStackTrace();
        }finally {
    ...
    public static String generateRandomIPv4() {
        ...
        for (int i = 0; i < 4; i++) {
            int segment = random.nextInt(256); // 每个段的范围是0到255
    ...
    public static void main(String[] args) {
        System.out.println(getIpArea("220.194.117.74"));
    }
