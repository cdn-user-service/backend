...
public class AntsCdnAuthController extends AbstractController {
    ...
    public R nodePageList(@RequestBody CdnClientQueryForm params){
        ...
        return R.ok().put("data", cdnSysAuthService.nodePageList(params)).put("curTask",StaticVariableUtils.curPushTaskName);
    }
    ...
    public R nodeDelete(@RequestParam Integer id){
        //删除无头节点
        List<CdnClientEntity> child_list=cdnClientDao.selectList(new QueryWrapper<CdnClientEntity>().eq("client_type",3));
        ...
        Integer count= cdnClientGroupChildConfDao.selectCount(new QueryWrapper<CdnClientGroupChildConfEntity>().eq("client_id",id));
        if(count>0){
    ...
    public R groupSave(@RequestBody Map param){
        ...
        if(null==inputGroup){return R.error("参数有误！");}
    ...
    public R groupCreate(@RequestBody Map param){
        ...
        if(null==group){return R.error("参数有误！");}
    ...
    public R deleteClientAreaList(@RequestBody Map param){
        ...
        if (param.containsKey("ids")){
            for (String id:param.get("ids").toString().split(",")){
                Integer count=cdnClientDao.selectCount(new QueryWrapper<CdnClientEntity>().eq("area_id",id));
                if (count>0){
    ...
    private void  removeClientGroupId(String id){
        ...
        if(null!=dnsConfEntity){
            //删除子
            List<CdnClientGroupChildConfEntity> list= cdnClientGroupChildConfDao.selectList(new QueryWrapper<CdnClientGroupChildConfEntity>().eq("parent_id",id));
            ...
            if(null!=client && null!=group){
                dnsCApiService.removeRecordByInfo(group.getDnsConfigId(),"*."+group.getHash(),"A",dnsConfEntity.getLine(),client.getClientIp(),null);
            }
    ...
    public R pushByInputInfo(@RequestBody Map params){
        String[] keys={PushTypeEnum.SHELL_ANTS_CMD_TO_MAIN.getName(), PushTypeEnum.SHELL_ANTS_CMD_TO_NODE.getName()};
    ...
    public R checkKeyShow(){
        ...
        if(StringUtils.isBlank(StaticVariableUtils.checkNodeInputToken)){
            if(StringUtils.isBlank(StaticVariableUtils.authMasterIp) || StringUtils.isBlank(StaticVariableUtils.masterWebSeverName)){
                return R.error("获取失败");
            }
        ...
        String url=String.format("url: '%s://%s%s'\n",StaticVariableUtils.MasterProtocol,StaticVariableUtils.masterWebSeverName, QuerySysAuth.CHECK_NODE_INPUT_PATH);
        try {
    ...
    private void updateNodeStableScoreByImportString(String sourceIp,String confStr,String nodeCheckResult){
            ...
            for (String ipCheck:nodeCheckResult.split(",")){
                ...
                if(2==info.length){
                    ...
                    if (!StaticVariableUtils.nodeCheckMap.containsKey(ip)){
                        ...
                        StaticVariableUtils.nodeCheckMap.get(ip).put(sourceIp,System.currentTimeMillis()+"#"+healthValue);
                    }
                }
            }
        }
        ...
        for (String nodeIp:StaticVariableUtils.nodeCheckMap.keySet()){
            ...
            if (null!=StaticVariableUtils.nodeCheckMap.get(nodeIp)){
                for (String timeStatusStr:StaticVariableUtils.nodeCheckMap.get(nodeIp).values()){
                    ...
                    if ((System.currentTimeMillis()-Long.parseLong(info[0]))>3*config.getCheckNodeFrequency()*1000){
                        //check date Invalid
                        continue;
                    }
                    total_sum++;
        ...
        if(normalNode.size()>0){
            ...
            updateWrapper.in("client_ip",normalNode.toArray()).set("check_time",System.currentTimeMillis()/1000).setSql(" stable_score= stable_score+1 ");
            cdnClientDao.update(null,updateWrapper);
        }
        if(unNormalNode.size()>0){
            UpdateWrapper<CdnClientEntity> updateWrapper=new UpdateWrapper<>();
            updateWrapper.in("client_ip",unNormalNode.toArray()).set("check_time",System.currentTimeMillis()/1000).setSql(" stable_score=0 ");
            cdnClientDao.update(null,updateWrapper);
        }
    }
    ...
    @SysLog("迁移主控")
    @RequiresPermissions("sys:ants_conf:save")
