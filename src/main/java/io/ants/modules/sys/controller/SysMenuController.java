...
//	}
	...
	private SysMenuEntity getChildPerms(SysMenuEntity m ,List<SysMenuEntity> menuList){
		List<SysMenuEntity> pid_p_list=menuList.stream().filter(f->f.getParentId().equals(m.getMenuId())).collect(Collectors.toList());
	...
	public R list(){
		...
		List<SysMenuEntity> menu_root_list=treeMenuList.stream().filter(t->t.getParentId().equals(0L)).collect(Collectors.toList());
