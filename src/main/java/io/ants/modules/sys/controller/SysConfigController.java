...
public class SysConfigController extends AbstractController {
	...
	public R info(@PathVariable("paramKey") String paramKey){
		SysConfigEntity config = sysConfigService.getOne(new QueryWrapper<SysConfigEntity>().eq("param_key",paramKey).last("limit 1"));
		return R.ok().put("config", config);
	}
	...
	public R get_redis_conf_view(){
		//yum install -y wget && wget -O install.sh http://download.antsxdp.com/nginx20/install.sh && sh install.sh -i 121.62.18.146 -p 8881130py
		...
		//String remote="1.29";
		//		Map<String,Object> rtMap= QueryAnts.queryAntsXdpNewestVersionData(goods,"1.10");
		...
		String iCmd="yum install -y wget && wget -O install.sh "+ QuerySysAuth.getDownloadInstallNodeAddressCentos7()+" && sh install.sh";
		nvo.setMaster_ip(StaticVariableUtils.authMasterIp);
		nvo.setInstall_node_command(iCmd);
		nvo.setFull_install_node_command(String.format("%s -i %s -p %s ",iCmd,StaticVariableUtils.authMasterIp,requirepass));

		//wget -O - http://download.antsxdp.com/nginx20/3.0/install.sh | sudo bash -s -- -i 121.62.17.153 -p 8881130py
		if (StringUtils.isNotBlank(QuerySysAuth.getDownloadInstallNodeAddressWithUbuntu())){
			//apt  install -y wget && wget -O install.sh http://download.antsxdp.com/nginx20/ubuntu/install.sh && sh install.sh -i 121.62.17.153 -p 8881130py
			String ubuntuICmd=String.format("apt  install -y wget && wget -O install.sh %s && sh install.sh -i %s -p %s",QuerySysAuth.getDownloadInstallNodeAddressWithUbuntu(),StaticVariableUtils.authMasterIp,requirepass );
