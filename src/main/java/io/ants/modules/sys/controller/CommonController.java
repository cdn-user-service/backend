...
public class CommonController extends AbstractController{
    ...
    public R enums(){
        ...
        try{
            ...
            orderMap.put("product_example","{\"m\":{\"value\":1,\"status\":1},\"s\":{\"value\":100,\"status\":1},\"y\":{\"value\":1000,\"status\":1}}");
            orderMap.put("buy_product_example","{\"type\":\"m\",\"sum\":1}");
            orderMap.put("renew_product_example","{\"serialNumber\":\"1650361752071001\",\"sum\":1}");
            orderMap.put("adder_product_example","{\"serialNumber\":\"1650361752071001\",\"startTime\":\"1650769152\",\"type\":\"m\",\"sum\":2}");
    ...
    public R webConfig(){
        Map<String,Object>map=new HashMap<>();
        SysConfigEntity sysConfig=sysConfigDao.selectOne(new QueryWrapper<SysConfigEntity>()  .eq("param_key", ConfigConstantEnum.WEB_SITE_CONFIG_KEY.getConfKey()) .last("limit 1"));
        map.put("web_config",sysConfig);
        SysConfigEntity sysConfig_dir=sysConfigDao.selectOne(new QueryWrapper<SysConfigEntity>()  .eq("param_key",ConfigConstantEnum.WEB_DIR_CONF.getConfKey()) .last("limit 1"));
        ...
        if(null!=sysConfig_dir){
            ...
            if (StringUtils.isNotBlank(dirStr)){
                ...
                if (null!=webDir && StringUtils.isNotBlank(webDir.getUserDir())){
                    ...
                    map.put("user_dir",StaticVariableUtils.masterWebSeverName +"/"+u_dir_s[u_dir_s.length-1]+"/");
                }
            }
        }

        List<SysConfigEntity> list=sysConfigDao.selectList(new QueryWrapper<SysConfigEntity>().select("id,param_key,status") );
        ...
        Integer wkCount=tbWorkOrderListDao.selectCount(new QueryWrapper<TbWorkOrderListEntity>().eq("status",0));
    ...
    public String init(){
        if (isCanInit()){
            ...
            String[] truncate_tables={"cdn_client","cdn_client_group","cdn_client_group_dns_conf","cdn_consume","cdn_ip_control","cdn_product","cdn_suit","schedule_job_log","sys_captcha","sys_log","sys_user_token","tb_cdn_public_mut_attr","tb_certify","tb_dns_config","tb_order","tb_pay_recory","tb_site","tb_site_attr","tb_site_mut_attr","tb_stream_proxy","tb_user","tb_rewrite"};
