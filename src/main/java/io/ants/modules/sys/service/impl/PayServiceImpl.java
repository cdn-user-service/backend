...
public class PayServiceImpl implements PayService {
    ...
    private void operate_recharge(TbOrderEntity order, TbPayRecordEntity record) {
        ...
        if (null != userId) {
            ...
            if (null != user) {
                ...
                tbUserDao.update(null, new UpdateWrapper<TbUserEntity>().eq("user_id", userId).set("property_balance",
                        user.getPropertyBalance() + order.getPayable()));
            } else {
    ...
    private void operate_cdn_buy(TbOrderEntity order, TbPayRecordEntity record) {
        ...
        if (1 == charging_mode_value) {
            ...
            try {
                Date eDate = ProductUnitEnum.getEndDate(buy_obj.getString("type"), new Date(),
                        buy_obj.getInteger("sum"));
    ...
    private void operate_cdn_renew(TbOrderEntity order, TbPayRecordEntity record) {
        ...
        Integer[] in_type_list = { OrderTypeEnum.ORDER_CDN_SUIT.getTypeId(),
    ...
    private void operate_cdn_adder(TbOrderEntity order, TbPayRecordEntity record) {
        ...
        if (!buy_obj.containsKey("serialNumber") || !buy_obj.containsKey("startTime") || !buy_obj.containsKey("type")
                || !buy_obj.containsKey("sum")) {
        ...
        CdnProductEntity product = DataTypeConversionUtil.json2entity(orderInitJson.getJSONObject("product_obj"),
        ...
        try {
            ...
            // JSONObject
            // final_attr_json=ProductAttrNameEnum.getFinalAttrJson(product_attr_s_json_array);
    ...
    private void operate_cdn_upgrade(TbOrderEntity order, TbPayRecordEntity record) {
        ...
        try {
            ...
            Integer[] in_type_list = { OrderTypeEnum.ORDER_CDN_SUIT.getTypeId(),
                    OrderTypeEnum.ORDER_CDN_RENEW.getTypeId(), OrderTypeEnum.ORDER_CDN_UPGRADE.getTypeId() };

            // 获取最后一个套餐的ATTR
            /*
             * CdnSuitEntity last_suit=cdnSuitDao.selectOne(new
             * QueryWrapper<CdnSuitEntity>()
             * .eq("user_id",order.getUserId())
             * .in("suit_type",in_type_list)
    ...
    private void operateRecordHandle() {
        ...
        try {
            List<TbPayRecordEntity> list = tbPayRecordDao
                    .selectList(new QueryWrapper<TbPayRecordEntity>().eq("operate_status", 0).last("limit 2"));
            ...
            for (TbPayRecordEntity record : list) {
                ...
                TbOrderEntity order = tbOrderDao.selectOne(new QueryWrapper<TbOrderEntity>()
                        .eq("serial_number", record.getSerialNumber()).last("limit 1"));
                if (null == order) {
                    // 未知订单
    ...
    public R orderPayBalance(PayBalanceForm form) {
        ...
        TbOrderEntity order = tbOrderDao.selectOne(
                new QueryWrapper<TbOrderEntity>().eq("serial_number", form.getSerialNumber()).last("limit 1"));
        if (null == order) {
        ...
        // }
        // if(
        // !user.getPropertyPayPassword().equals(DigestUtils.sha256Hex(form.getPassword()))
        // ){
        // return R.error("密码错误");
        // }
        // }
    ...
    @Override
    public void payResultSub(String SerialNumber, Integer payPaid, String out_trade_id, Integer payType,
            String payMsg) {
        try {
            // 只记录
    ...
    public String aliPayNoticeCallback(HttpServletRequest request) {
        ...
        try {
            ...
            // 调用SDK验证签名
            boolean signVerified = AlipaySignature.rsaCheckV1(params, alipayConfig.getAlipayPublicKey(), charset,
                    sign_type);
            if (signVerified) {
                // 支付宝回调签名认证成功
            ...
            // logger.error("支付宝回调签名认证失败 2,paramsJson:{},errorMsg:{}", paramsJson,
            // e.getMessage());
            e.printStackTrace();
    ...
    public void allinpayCallback(HttpServletRequest request, HttpServletResponse response) {
        try {
            ...
            try {
                ...
                // todo 验签完毕进行业务处理
                if (isSign) {
                    if (params.containsKey("cusorderid") && params.containsKey("trxamt")
                            && params.containsKey("trxid")) {
                        ...
                        this.payResultSub(sn, amount.intValue(), outTradeId, payType,
                                DataTypeConversionUtil.map2json(params).toJSONString());
                    }
                }
    ...
    private void alipayCheck(Map<String, String> params) throws AlipayApiException {
        try {
            ...
            // long total_amount = new BigDecimal(params.get("total_amount")).multiply(new
            // BigDecimal(100)).longValue();
            // if (total_amount != order.getAmount().multiply(new
            // BigDecimal(100)).longValue()) {
            // throw new AlipayApiException("error RSAcontent ,["+total_amount
    ...
    private String GetWxpay(HttpServletRequest request, String outTradeNo, String totalAmount, String body,
            String trade_type) {
        try {
            // String code_url="";
    ...
    @Override
    public Map<String, String> fuiouPay(String ip, String mchnt_order_no, String order_amt, String goods_des,
            String order_type) {
        try {
            ...
            reqs.put("random_str", generateSalt(16));
            reqs.put("sign", "");
            ...
            // reqs.put("mchnt_order_no",
            // Const.order_prefix+sdf_no.format(calendar.getTime()) +
