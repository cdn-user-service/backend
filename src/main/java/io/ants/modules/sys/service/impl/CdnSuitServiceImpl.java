...
public class CdnSuitServiceImpl implements CdnSuitService {
    ...
    public PageUtils getProductList(QueryCdnProductForm form) {
        ...
        IPage<CdnProductEntity> page = cdnProductDao.selectPage(
                ...
                new QueryWrapper<CdnProductEntity>()
                        ...
                        .ne(null!=form.getUserType()&& UserTypeEnum.USER_TYPE.getId().equals(form.getUserType()),"status",ProductStatusEnum.ONLY_FIRST.getId())
    ...
    public CdnProductEntity saveProduct(Map<String, Object> params) {
        ...
        //CdnProductEntity productEntity= objectMapper.convertValue(params, io.cdn.modules.sys.entity.CdnProductEntity.class);
       ...
       //ProductAttrArrayVo attrArrayVo=
            ...
            for (String gid:productEntity.getServerGroupIds().split(",")){
                ...
                if(null==dnsConfId){
                    ...
                    if(!dnsConfId.equals(groupEntity.getDnsConfigId())){
                        throw new RRException("同一产品对应的节点业务分组DNS需一致！分组ID:["+gid+"]的DNS接口Id须是["+dnsConfId+"]！当前的分组ID:["+gid+"]的DNS接口Id是["+groupEntity.getDnsConfigId()+"]");
                    }
                }
            }
        }
        ...
        if (productEntity.getProductType().equals(OrderTypeEnum.ORDER_CDN_SUIT.getTypeId() ) &&  productEntity.getStatus().equals(ProductStatusEnum.ONLY_FIRST.getId())){
            OrderCdnProductVo productVo=DataTypeConversionUtil.string2Entity(params.get("productJson").toString(),OrderCdnProductVo.class);
            if (null==productVo){
    ...
    public CdnProductAttrEntity SaveProductAttr(Map<String, Object> params) {
        //ObjectMapper objectMapper = new ObjectMapper();
        //CdnProductAttrEntity productAttrEntity= objectMapper.convertValue(params, io.cdn.modules.sys.entity.CdnProductAttrEntity.class);
        JSONObject jsonObject=DataTypeConversionUtil.map2json(params);
        CdnProductAttrEntity productAttrEntity=DataTypeConversionUtil.json2entity(jsonObject, CdnProductAttrEntity.class);
        if( null!=productAttrEntity.getId()  && 0!=productAttrEntity.getId() ){
            ...
            if(null!=t_product_attr){
                //修改后返回
                Integer count=cdnProductAttrDao.selectCount(new QueryWrapper<CdnProductAttrEntity>().ne("id",t_product_attr.getId()).eq("name",t_product_attr.getName()).eq("is_delete",0));
                if(0!=count){
        ...
        //增加
        Integer count=cdnProductAttrDao.selectCount(new QueryWrapper<CdnProductAttrEntity>() .eq("name",productAttrEntity.getName()).eq("is_delete",0));
        if(0==count){
            ProductAttrNameEnum obj=ProductAttrNameEnum.getEnum(productAttrEntity.getName());
            if(null!=obj){
                productAttrEntity.setName(obj.getAttr());
    ...
    * 创建充值订单*/
     ...
     * @return
     */
    private R buy_pre_payment(OrderCdnNewInitVo initVo, CdnProductEntity product, TbOrderEntity tbOrderEntity, SubmitOrderForm submitOrderForm){
        String eMsg="";
    ...
    * 创建购买CDN套餐*/
    private R createCdnSuitOrder(SubmitOrderForm submitOrderForm){
        ...
        OrderCdnNewInitVo initVo=DataTypeConversionUtil.string2Entity(submitOrderForm.getInitJson(), OrderCdnNewInitVo.class);
    ...
    private R createCdnRenewOrder(SubmitOrderForm submitOrderForm){
        ...
        //JSONObject buyJsonObject=DataTypeConversionUtil.string2Json(submitOrderForm.getInitJson());
        OrderCdnRenewVo cdnRenewVo= DataTypeConversionUtil.string2Entity(submitOrderForm.getInitJson(),OrderCdnRenewVo.class);
        if(null==cdnRenewVo){
        ...
        //主套餐信息
        Integer[] canQueryType={CdnSuitStatusEnum.UNKNOWN.getId(),CdnSuitStatusEnum.NORMAL.getId(),CdnSuitStatusEnum.TIMEOUT.getId()};
        ...
        //从原套餐订单中产品中获取价格相关数据
        //原套餐的价格 {"m":{"value":1,"status":0},"s":{"value":10100,"status":0},"y":{"value":20100,"status":1}}
    ...
    * CDN 增值服务*/
    private R create_cdn_extra_order(SubmitOrderForm submitOrderForm){
        ...
        //JSONObject buyJson=DataTypeConversionUtil.string2Json(submitOrderForm.getInitJson());
        OrderCdnAddedInitVo initVo=DataTypeConversionUtil.string2Entity(submitOrderForm.getInitJson(),OrderCdnAddedInitVo.class);
        //{"serialNumber":"1650361752071001","startTime":"1650769152","type":"m","sum":2}
        ...
        if (suitEndDate.before(orderEndDate)){
            ...
            //套餐在增值前到期--折算价格
            payAble=new Long(payAble*(suitEndDate.getTime()-startDate.getTime())/(orderEndDate.getTime()-startDate.getTime())).intValue();
            tbOrderEntity.setPayable(payAble);
        }else{
            //全价
            tbOrderEntity.setPayable(payAble);
    ...
    private  R createCdnUpgradeOrder(SubmitOrderForm submitOrderForm){
        ...
        OrderCdnUpgradeVo cdnUpgradeVo=DataTypeConversionUtil.string2Entity(submitOrderForm.getInitJson(),OrderCdnUpgradeVo.class);
        //JSONObject buyJsonObject=DataTypeConversionUtil.string2Json(submitOrderForm.getInitJson());
        ...
        Integer[] enable_up_list={OrderTypeEnum.ORDER_CDN_SUIT.getTypeId(),OrderTypeEnum.ORDER_CDN_UPGRADE.getTypeId()};
        CdnSuitEntity source_suit=suitDao.selectOne(new QueryWrapper<CdnSuitEntity>()
                .eq("serial_number",serialNumber)
        ...
        if (sourceOrderInitJson.containsKey("product_obj")){
            ...
            CdnProductEntity source_product=DataTypeConversionUtil.json2entity(source_product_json, CdnProductEntity.class);
    ...
    public R createOrder(SubmitOrderForm submitOrderForm) {
        String eMsg="";
        if(null==submitOrderForm.getUserId() || null==submitOrderForm.getOrderType() || null==submitOrderForm.getTargetId()){
        ...
        switch (orderEntity.getOrderType()){
                    ...
                    if (StringUtils.isNotBlank(orderEntity.getInitJson())){
                        ...
                        if (null!=init_json && init_json.containsKey("product_obj")){
                            ...
                            if (product_obj.containsKey("name")){
                                productObj.put("name","["+orderEntity.getTargetId()+"]"+product_obj.getString("name"));
                            }
                        }
                    }
                }
            ...
            case 30:
                if (true){
                    if (StringUtils.isNotBlank(orderEntity.getInitJson())){
                        JSONObject init_json=DataTypeConversionUtil.string2Json(orderEntity.getInitJson());
                        if (null!=init_json && init_json.containsKey("date") && init_json.containsKey("product_obj")){
                            String date=init_json.getString("date");
                            JSONObject product_obj=init_json.getJSONObject("product_obj");
                            if (product_obj.containsKey("name")){
                                productObj.put("name","["+orderEntity.getTargetId()+"]"+product_obj.getString("name")+"-"+date);
                            }
                        }
                    }
                }
    ...
    public PageUtils orderList(Map params) {
        ...
        i_page.getRecords().forEach(item->{
            TbPayRecordEntity record=recordDao.selectOne(new QueryWrapper<TbPayRecordEntity>().eq("serial_number",item.getSerialNumber()).last("limit 1"));
    ...
    private CdnProductEntity getProductEntityBySuit(CdnSuitEntity suit){
        ...
        TbOrderEntity order=tbOrderDao.selectOne(new QueryWrapper<TbOrderEntity>().eq("serial_number",suit.getPaySerialNumber()).last("limit 1"));
        if(null!=order){
        ...
        TbOrderEntity orderMain=tbOrderDao.selectOne(new QueryWrapper<TbOrderEntity>().eq("serial_number",suit.getSerialNumber()).last("limit 1"));
        if (null!=orderMain){
    ...
    public PageUtils suitList(Map params) {
        ...
        if (s_MODE_ALL.equals(mode)){
            ...
            ipage.getRecords().forEach(item->{
                //CdnSuitEntity t_suit=commonTaskService.GetSuitDetail(item.getUserId(),item.getSerialNumber());
    ...
    private void updateCdnSuit(CdnSuitEntity item,Integer mode){
        if (S_MODE_IN_USE.equals(mode))  {
            ...
            if (item.getSuitType().equals(OrderTypeEnum.ORDER_CDN_ADDED.getTypeId())){
                ...
                Integer[] sType={OrderTypeEnum.ORDER_CDN_SUIT.getTypeId(),OrderTypeEnum.ORDER_CDN_RENEW.getTypeId()};
                CdnSuitEntity main_suit=commonTaskService.commGetSuitDetail(item.getUserId(),item.getSerialNumber(),Arrays.asList(sType),true);
                ...
                //LocalDate lastDay = now_date.with(TemporalAdjusters.lastDayOfMonth()); // 获取当前月的最后一天
                LocalDate lastDay = now_date;//最后一天为当天
                Date fd=DateUtils.stringToDate(firstDay.format(DateTimeFormatter.ISO_DATE)+" 00:00:00",DateUtils.DATE_TIME_PATTERN) ;
                Date ld=DateUtils.stringToDate(lastDay.format(DateTimeFormatter.ISO_DATE)+" 23:59:59",DateUtils.DATE_TIME_PATTERN) ;
                item.setCurrentMonthData(commonTaskService.getBytesDataListBySuitSerialNumber(item.getUserId(),item.getSerialNumber(),fd,ld));
            }
    ...
    private void simpleUpdateSuit(CdnSuitEntity suit){
        ...
        //3 更新用户名
        TbUserEntity user=userDao.selectOne(new QueryWrapper<TbUserEntity>().eq("user_id",suit.getUserId()).select("username"));
        if (null!=user){
    ...
    public R allSuitListByUser(Long userId,Integer all) {
         ...
         List<CdnSuitEntity> list=suitDao.selectList(new QueryWrapper<CdnSuitEntity>()
                 .eq("suit_type",OrderTypeEnum.ORDER_CDN_SUIT.getTypeId())
                 .eq("user_id",userId)
         ...
         list.forEach(item->{
             ...
             if (null!=item.getAttr()){
                 ...
                 if (null!=item.getAttr().getSite()){
                     int count=siteDao.selectCount(new QueryWrapper<TbSiteEntity>().eq("serial_number", sVo.getSerial_number()));
    ...
    public boolean cancellationSuit(Long userId, String serialNumber) {
        //注销套餐->{套餐 续费 增值}
        Integer count=cdnSuitDao.selectCount(new QueryWrapper<CdnSuitEntity>().eq(null!=userId,"user_id",userId).eq("serial_number",serialNumber) );
        if (0==count){
            return false;
        }
        //套餐绑定有站点 forward 则失败
        Integer count1=siteDao.selectCount(new QueryWrapper<TbSiteEntity>().eq("serial_number",serialNumber));
        if (count1>0){
            throw new RRException("注销失败，已绑定了站点的套餐不可注销");
        }
        Integer count2=tbStreamProxyDao.selectCount(new QueryWrapper<TbStreamProxyEntity>().eq("serial_number",serialNumber));
        if (count2>0){
        ...
        for (CdnSuitEntity suit:list){
            ...
            if (null!=payOrderEntity){
                ...
                if (suit.getStartTime().before(new Date())){
                    //Integer total_month=DateUtils.getIntervalMonth(suit.getStartTime(),suit.getEndTime());
                    // Integer surplus_month=DateUtils.getIntervalMonth(new Date(),suit.getEndTime());
    ...
    public R user_bytes_detail( QuerySuitBytes querySuitBytes){
        return R.ok().put("data",commonTaskService.getBytesDataListBySuitSerialNumber(null,querySuitBytes.getSerialNumber(), DateUtils.LongStamp2Date(querySuitBytes.getStartTime()) ,DateUtils.LongStamp2Date(querySuitBytes.getEndTime())));
    }
    ...
    public void reInitDbColumns() {
        ...
        List<CdnSuitEntity>list=cdnSuitDao.selectList(new QueryWrapper<CdnSuitEntity>().isNull("flow").eq("status",CdnSuitStatusEnum.NORMAL.getId()));
        for (CdnSuitEntity suit:list){
