...
public class AppProductController {
    ...
    @Operation(summary = "我购买的套餐[分页]")
    public R suitList(@Parameter(hidden = true) @RequestAttribute("userId") Long userId,
            @RequestBody SuitListForm form) {
    ...
    @Operation(summary = "我购买的套餐[不分页]")
    public R listByUser(@Parameter(hidden = true) @RequestAttribute("userId") Long userId, Integer all) {
        return cdnSuitService.allSuitListByUser(userId, all);
    }

    @GetMapping("/by_access/suit/listbyuser")
    @Operation(summary = "access我购买的套餐[不分页]")
    ...
    public R suitDetail(@Parameter(hidden = true) @RequestAttribute("userId") Long userId,
    ...
    public R order_list(@Parameter(hidden = true) @RequestAttribute("userId") Long userId,
    ...
    @Operation(summary = "根据类型获取所有可购套餐")
    ...
    @UserLog("创建订单")
    public R order_create(@Parameter(hidden = true) @RequestAttribute("userId") Long userId,
            @RequestBody SubmitOrderForm form) {
    ...
    @Operation(summary = "创建stripe支付链接")
    @UserLog("创建stripe支付链接")
    public R paymentLinkCreate(@Parameter(hidden = true) @RequestAttribute("userId") Long userId,
            @RequestParam String serialNumber,
    ...
    @Operation(summary = "创建allinpay")
    @UserLog("创建allinpay")
    public R allinpayCreate(@Parameter(hidden = true) @RequestAttribute("userId") Long userId,
            @RequestParam String serialNumber,
            @RequestParam String payMethod) {
    ...
    @Operation(summary = "创建tokenpay支付")
    @UserLog("创建tokenpay支付")
    public R tokenpayCreate(@Parameter(hidden = true) @RequestAttribute("userId") Long userId,
            @RequestParam String serialNumber,
    ...
    @Operation(summary = "创建彩虹易支付")
    @UserLog("创建彩虹易支付")
    public R cccyunPayCreate(@Parameter(hidden = true) @RequestAttribute("userId") Long userId,
            @RequestParam String serialNumber,
            @RequestParam String payMethod) {
        if (StringUtils.isBlank(serialNumber)) {
            return R.error("serialNumber is null");
    ...
    private Integer getUserAuthMode(String orderSerialNumber) {
        SysConfigEntity sysConfig = sysConfigDao.selectOne(new QueryWrapper<SysConfigEntity>()
                .and(q -> q.eq("param_key", ConfigConstantEnum.ALIPAYUSERCERTIFY_CONFIG_KEY.getConfKey()).or()
                        .eq("param_key", ConfigConstantEnum.TENCENTUSERCERTIFY_CONFIG_KEY.getConfKey()))
                .eq("status", 1).last("limit 1"));
        if (null == sysConfig) {
            throw new RRException("实名未开启--[1]！");
        }

        TbPayRecordEntity record = tbRecordDao.selectOne(
                new QueryWrapper<TbPayRecordEntity>().eq("serial_number", orderSerialNumber).last("limit 1"));
        if (null != record) {
            if (record.getPayType().equals(PayTypeEnum.PRO_TYPE_alipay.getId())) {
                ...
                } else if (sysConfig.getParamKey()
                        .equals(ConfigConstantEnum.TENCENTUSERCERTIFY_CONFIG_KEY.getConfKey())) {
                ...
                if (sysConfig.getParamKey().equals(ConfigConstantEnum.TENCENTUSERCERTIFY_CONFIG_KEY.getConfKey())) {
                    return WECHATPAY_AUTH_MODE;
                        ...
                        .equals(ConfigConstantEnum.ALIPAYUSERCERTIFY_CONFIG_KEY.getConfKey())) {
                    return ALIPAY_AUTH_MODE;
                }
            } else {
                if (sysConfig.getParamKey().equals(ConfigConstantEnum.ALIPAYUSERCERTIFY_CONFIG_KEY.getConfKey())) {
    ...
    private List<SysConfigEntity> pay_list() {
        ...
        SysConfigEntity sysConfig = sysConfigDao.selectOne(new QueryWrapper<SysConfigEntity>()
                .eq("param_key", ConfigConstantEnum.PAY_IDS.getConfKey()).eq("status", 1).last("limit 1"));
        if (null != sysConfig) {
            ...
            if (pay_select_json.containsKey("ids")) {
                ...
                for (String id : ids) {
                    SysConfigEntity pay_conf = sysConfigDao.selectOne(new QueryWrapper<SysConfigEntity>().eq("id", id)
                            .eq("status", 1).select("id,param_key,remark"));
                    if (null != pay_conf) {
        ...
        try {
            ...
            if (StringUtils.isNotBlank(result) && isJSON(result)) {
                ...
                if (ali_result_json.containsKey("alipay_user_certify_open_query_response")) {
                    JSONObject alipay_user_certify_open_query_response_obj = ali_result_json
                            .getJSONObject("alipay_user_certify_open_query_response");
                    if (alipay_user_certify_open_query_response_obj.containsKey("code")
                            && alipay_user_certify_open_query_response_obj.containsKey("passed")) {
    ...
    private R tencentQueryAuth(JSONObject payJsonObject, TbUserEntity user, TbOrderEntity alreadyCompleteOrder) {
        ...
        if (bizcodores.containsKey("BizToken")) {
            ...
            if (StringUtils.isNotBlank(result) && isJSON(result)) {
                ...
                if (text_json.containsKey("ErrCode") && text_json.get("ErrCode") != null
                        && 0 == text_json.getInteger("ErrCode")) {
    ...
    public R userAuth(@Parameter(hidden = true) @RequestAttribute("userId") Long userId,
            @RequestBody UserCertifyForm form) {
        ...
        if (null != user.getRealnameStatus() && 1 == user.getRealnameStatus()) {
            // AppUserEntity userEntity=appUserDao.selectOne(new
            // QueryWrapper<AppUserEntity>().eq("user_id",user.getUserId()).select("realname_status,realname_name,realname_certificatetype,realname_certificateid,realname_mode,realname_time"));
        ...
        for (TbOrderEntity order : auth_order_list) {
            if ((now.getTime() - order.getCreateTime().getTime()) < (AUTH_TIME_OUT)
                    && order.getStatus() == PayStatusEnum.PAY_COMPLETE.getId()) {
                ...
                not_pay_order = order;
                    ...
                    && order.getStatus() == PayStatusEnum.PAY_NOT_PAY.getId()) {
        ...
        if (null != alreadyCompleteOrder) {
            ...
            if (null != payJsonObject && payJsonObject.containsKey("mode")) {
                ...
                if (StringUtils.isBlank(form.getCert_name()) || StringUtils.isBlank(form.getCert_no())) {
                    ...
                    tbOrderDao.updateById(alreadyCompleteOrder);
                    R r3 = this.userAuthHandle(alreadyCompleteOrder.getSerialNumber(), form.getCert_name(),
                            form.getCert_no());
                    if (1 == r3.getCode()) {
    ...
    @Operation(summary = " 用户套餐流量使用详情")
    public R user_bytes_detail(@Parameter(hidden = true) @RequestAttribute("userId") Long userId,
            @RequestBody QuerySuitBytes querySuitBytes) {
        logger.info("##### app ELK ######");
        return R.ok().put("data",
                commonTaskService.getBytesDataListBySuitSerialNumber(userId, querySuitBytes.getSerialNumber(),
    ...
    @Operation(summary = "使用后付费")
    @UserLog("使用后付费")
    public R postpaid_create(@Parameter(hidden = true) @RequestAttribute("userId") Long userId,
        ...
        if (2 == chargingModeValue || 3 == chargingModeValue) {
            ...
            // LogTypeEnum.PRODUCT_LOG.getId(),this.getClass().getName()+"/"+Thread.currentThread().getStackTrace()[1].getMethodName(),"");
        } else {
    ...
    @Operation(summary = "注销套餐|删除套餐")
    @UserLog("使用后付费")
    public R cancellationSuit(@Parameter(hidden = true) @RequestAttribute("userId") Long userId,
            @RequestParam String SerialNumber) {
        boolean result = cdnSuitService.cancellationSuit(userId, SerialNumber);
        if (result) {
            // tbLogService.FrontUserWriteLog(userId,
            // LogTypeEnum.PRODUCT_LOG.getId(),this.getClass().getName()+"/"+Thread.currentThread().getStackTrace()[1].getMethodName(),SerialNumber);
            return R.ok();
        } else {
            return R.error("失败！");
        }
    ...
    @Operation(summary = "清算后付费套餐")
    @UserLog("清算后付费套餐")
    public R liquidationSuit(@Parameter(hidden = true) @RequestAttribute("userId") Long userId,
            @RequestParam String SerialNumber) {
    ...
    @Operation(summary = "目标套餐可升级的套餐")
    public R canUpdate(@Parameter(hidden = true) @RequestAttribute("userId") Long userId,
            @RequestParam Integer productId) {
        ...
        // 套餐收费方式
        JSONObject chargingModeObj = getAttrObjByJsonArray(attrArray, "attr",
                ProductAttrNameEnum.ATTR_CHARGING_MODE.getAttr());
        ...
        if (true) {
            ...
            for (CdnProductEntity c_product : productList) {
                ...
                // 套餐收费方式
                JSONObject c_chargingModeObj = getAttrObjByJsonArray(c_attrArray, "attr",
                        ProductAttrNameEnum.ATTR_CHARGING_MODE.getAttr());
                if (null == chargingModeObj) {
                    continue;
                }
    ...
    @Operation(summary = "目标套餐绑定的站点")
    public R suit_site_bind(@Parameter(hidden = true) @RequestAttribute("userId") Long userId, String SerialNumber) {
