...
public class AppCommonController {
    ...
    @ApiOperation("获取公告消息[key1=titile,key2=content]")
    ...
    @ApiOperation("获取CDN产品列表[key1=product_name]")
    ...
    public R getWebConf(){
        ...
        SysConfigEntity dnsSiteConf=sysConfigDao.selectOne(new QueryWrapper<SysConfigEntity>().eq("param_key", ConfigConstantEnum.DNS_USER_API_ROOT_URI.getConfKey()).last("limit 1"));
        ...
        TbCdnPublicMutAttrEntity publicMutAttr= tbCdnPublicMutAttrDao.selectOne(new QueryWrapper<TbCdnPublicMutAttrEntity>()
        ...
        if (null==publicMutAttr){
            ...
            DnsRewriteConfVo vo= DataTypeConversionUtil.string2Entity(publicMutAttr.getPvalue(),DnsRewriteConfVo.class);
            allMap.put("dns_rewrite",vo);
        }
    ...
    @ApiOperation("获取注册协议")
    public R getAgent(){
        //SysConfigEntity web_agent_config=sysConfigDao.selectOne(new QueryWrapper<SysConfigEntity>().eq("param_key","WEB_AGREEMENT").last("limit 1") );
        return R.ok().put("data",sysConfigDao.queryByKey("WEB_AGREEMENT"));
    }


    @GetMapping("/web/update/log")
    @ApiOperation("cdn 更新日志")
