...
public class AppPayController {
    ...
    //@ApiOperation("alipay Notice Callback")
    ...
    //@ApiOperation("alipay Return Callback")
    ...
    public  R wechatPay(@RequestBody WechatPaySumbitOrderForm form) {
        ...
        if(StringUtils.isNotBlank(form.getMode())){
            ...
            if("native".equals(form.getMode())){
                 qrCodeUrl=payService.wxPayNativePay(request,form.getOutTradeNo(),form.getTotalAmount(),form.getBody());
            }else if("mweb".equals(form.getMode())){
                qrCodeUrl=payService.wxPayMWebPay(request,form.getOutTradeNo(),form.getTotalAmount(),form.getBody());
            }
    ...
    public R fuiouPay(@RequestBody FuiouPayForm form)  {
        ...
        return R.ok().put("data",payService.fuiouPay(ip,form.getOutTradeNo(),form.getTotalAmount(),form.getBody(),form.getMode()));
    }

    @PostMapping(value = "/fuioupayCallback" )
    //@ApiOperation("富友支付 CALLBACK")
    ...
    //@ApiOperation("富友支付 查询订单")
    public R fuiouPayQuery(String order_no, String order_type)  {
        if(null==order_no|| null==order_type){return R.error(403,"parat is error");}
    ...
    @ApiOperation("余额支付")
    @UserLog("余额支付")
    public R balancePay(@ApiIgnore @RequestAttribute("userId") Long userId, @RequestBody PayBalanceForm form){
        ...
        //tbLogService.FrontUserWriteLog(userId, LogTypeEnum.FINANCE_LOG.getId(),this.getClass().getName()+"/"+Thread.currentThread().getStackTrace()[1].getMethodName(),"");
    ...
    @ApiOperation("订单详情")
    public R order_detail(@ApiIgnore @RequestAttribute("userId") Long userId,@RequestParam String SerialNumber){
