...
public class AppSiteController {
    ...
    @PostMapping("/list")
    @ApiOperation("站点列表")
    public R siteList(@ApiIgnore @RequestAttribute("userId") Long userId, @RequestBody QuerySitePageForm form){
    ...
    @Login
    @GetMapping("/list/all")
    @ApiOperation("所有站点列表【仅SiteName】")
    public R siteListAll(@ApiIgnore @RequestAttribute("userId") Long userId,@RequestParam Integer igg){
       // igg=0 去除已经分配的
    ...
    @ApiOperation("获取域名分组")
    public R siteGroupList(@ApiIgnore @RequestAttribute("userId") Long userId,@RequestBody QuerySiteGroupForm form){
    ...
    @ApiOperation("保存域名分组")
    public R saveSiteGroup(@ApiIgnore @RequestAttribute("userId") Long userId,@RequestBody TbSiteGroupEntity tbSiteGroup){
    ...
    @ApiOperation("删除域名分组")
    public R deleteSiteGroup(@ApiIgnore @RequestAttribute("userId") Long userId,@RequestBody DeleteIdsForm form){
    ...
    @ApiOperation("修改站点状态")
    @UserLog("修改站点状态")
    public R changeSiteStatus(@ApiIgnore @RequestAttribute("userId") Long userId, Integer siteId, Integer status){
    ...
    @ApiOperation("获取站点")
    public R getSiteInfo(@ApiIgnore @RequestAttribute("userId") Long userId ,@RequestParam  String name){
    ...
    @ApiOperation("修改套餐")
    @UserLog("修改套餐")
    public R change_suit(@ApiIgnore @RequestAttribute("userId") Long userId,@RequestBody Map<String, Object> params){
    ...
    @ApiOperation("创建站点")
    @UserLog("创建站点")
    public R siteCreate(@ApiIgnore @RequestAttribute("userId") Long userId, @RequestBody Map map){
        //tbLogService.FrontUserWriteLog(userId, LogTypeEnum.OPERATION_LOG.getId(),this.getClass().getName()+"/"+Thread.currentThread().getStackTrace()[1].getMethodName(),"");
    ...
    public R testCreateSite(@RequestBody CreateSiteByTestUserForm form){
        ...
        TbUserEntity userEntity=tbUserDao.selectOne(new QueryWrapper<TbUserEntity>().eq("username",userName).last("limit 1"));
        int newUserFlag=0;
        if (null==userEntity){
        ...
        return  tbSiteServer.testCreateSite(userEntity.getUserId(), form.getMainServerName(),form.getSProtocol(),newUserFlag);
    }

    @PostMapping("/by_access/create")
    @ApiOperation("access创建站点")
    @UserLog("access创建站点")
    public R siteCreate(@RequestBody CreateSiteByAccessForm form){
        ...
        TbSiteEntity site= tbSiteServer.createSite(userEntity.getUserId(),form.getSerialNumber(),mainDomain,alias,sIp,sPort,form.getSProtocol());
        //tbLogService.FrontUserWriteLog(userId, LogTypeEnum.OPERATION_LOG.getId(),this.getClass().getName()+"/"+Thread.currentThread().getStackTrace()[1].getMethodName(),"");
        return R.ok().put("data",site);
    }
    ...
    @ApiOperation("批量创建站点")
    @UserLog("批量创建站点")
    public R batcreate(@ApiIgnore @RequestAttribute("userId") Long userId,@RequestBody Map map){
    ...
    @ApiOperation("删除站点")
    @UserLog("删除站点")
    public R delete(@ApiIgnore @RequestAttribute("userId") Long userId,@RequestBody Map<String, Object> params){
        if(params.containsKey("ids")){
            ...
            if(StringUtils.isNotBlank(ids)){
                ...
                for(String siteId:id_str){
                    ...
                    if (null!=site && null!=site.getStatus() &&  CdnSiteStatusEnum.CLOSE_AND_LOCKED.getId() ==site.getStatus()){
    ...
    @ApiOperation("删除站点属性")
    @UserLog("删除站点属性")
    public R delete_attr(@ApiIgnore @RequestAttribute("userId") Long userId,@RequestBody Map<String, Object> params){
    ...
    @ApiOperation("生成规则")
    @UserLog("生成规则")
    public R createSysWafRule(@ApiIgnore @RequestAttribute("userId") Long userId,@RequestBody SysCreateWafRuleForm form){
        ValidatorUtils.validateEntity(form);
    ...
    @PostMapping("/detail")
    @ApiOperation("站点详情")
    public R detail(@ApiIgnore @RequestAttribute("userId") Long userId,@RequestBody Map<String, Object> params){
    ...
    @ApiOperation("保存站点属性")
    @UserLog("保存站点属性")
    public R saveAttr(@ApiIgnore @RequestAttribute("userId") Long userId,@RequestBody Map<String, Object> params){
    ...
    @ApiOperation("批量修改站点属性")
    @UserLog("批量修改站点属性")
    public R batchModifySiteAttr(@ApiIgnore @RequestAttribute("userId") Long userId,@RequestBody BatchModifySiteAttrForm form){
        ValidatorUtils.validateEntity(form);
    ...
    @ApiOperation("批量添加站点属性")
    @UserLog("批量添加站点属性")
    public R batchAddSiteAttr(@ApiIgnore @RequestAttribute("userId") Long userId,@RequestBody BatchAddSiteAttrForm form){
        ValidatorUtils.validateEntity(form);
    ...
    @ApiOperation("批量匹配修改站点属性")
    @UserLog("批量匹配修改站点属性")
    public R batchSearchModifySiteAttr(@ApiIgnore @RequestAttribute("userId") Long userId,@RequestBody BatchSearchModifySiteAttrForm form){
        ValidatorUtils.validateEntity(form);
    ...
    @ApiOperation("修改站点属性状态")
    @UserLog("修改站点属性状态")
    public R ChangeAttrStatus(@ApiIgnore @RequestAttribute("userId") Long userId,@RequestBody ChangeSiteAttrStatusForm form){
        form.setUserId(userId);
        //tbLogService.FrontUserWriteLog(userId, LogTypeEnum.OPERATION_LOG.getId(),this.getClass().getName()+"/"+Thread.currentThread().getStackTrace()[1].getMethodName(),"");
        Integer result= tbSiteServer.changeAttrStatus(form.getUserId(),form.getSiteId(),form.getPkey(),form.getAttrId(),form.getStatus());
    ...
    @UserLog("修改站点属性排序")
    @ApiOperation("修改站点属性排序")
    public R ChangeAttrWeight(@ApiIgnore @RequestAttribute("userId") Long userId,@RequestBody ChangeSiteAttrWeightForm form){
       form.setUserId(userId);
       //tbLogService.FrontUserWriteLog(userId, LogTypeEnum.OPERATION_LOG.getId(),this.getClass().getName()+"/"+Thread.currentThread().getStackTrace()[1].getMethodName(),"");
    ...
    @ApiOperation("query elk")
    public R queryElk(@ApiIgnore @RequestAttribute("userId") Long userId, @RequestBody QueryElkForm params ){
    ...
    private void recordInfo(RedisUtils redisUtils){
        if(StringUtils.isBlank(StaticVariableUtils.masterWebSeverName)){
            ...
            if(null!=request){
                ...
                String f_str=StaticVariableUtils.masterWebSeverName +":"+StaticVariableUtils.MasterWebPort+"/antsxdp";
                ...
                if(StringUtils.isBlank(StaticVariableUtils.checkNodeInputToken) && StringUtils.isNotBlank(StaticVariableUtils.authMasterIp)){
    ...
    @ApiOperation("缓存清理")
    @UserLog("缓存清理")
    public R purge_cache(@ApiIgnore @RequestAttribute("userId") Long userId,@RequestBody Map params){
    ...
    @ApiOperation("解封IP")
    public R releaseInterceptIps(@ApiIgnore @RequestAttribute("userId") Long userId, @RequestBody Map map){
    ...
    @ApiOperation("缓存预热")
    public R pullCache(@ApiIgnore @RequestAttribute("userId") Long userId, @RequestBody Map param){
    ...
    public R pushByInputInfo(@RequestBody Map params){
        String[] keys={"site_select_chunk","apply_certificate","apply_certificate_v2", PushTypeEnum.SHELL_ANTS_CMD_TO_MAIN.getName(), PushTypeEnum.SHELL_ANTS_CMD_TO_NODE.getName()};
    ...
    @ApiOperation("批量重签")
    public R batReissued(@ApiIgnore @RequestAttribute("userId") Long userId,@RequestBody BatReissuedForm form ){
    ...
    @ApiOperation("批量导出")
    public R batExport(@ApiIgnore @RequestAttribute("userId") Long userId,@RequestBody BatExportForm form){
    ...
    @ApiOperation("预取缓存")
    public R prefetchCacheList(@ApiIgnore @RequestAttribute("userId") Long userId,@RequestBody QuerySiteCachePrefetchPageForm form ){
    ...
    @ApiOperation("预取缓存保存")
    public R prefetchCacheSave(@ApiIgnore @RequestAttribute("userId") Long userId,@RequestBody TbSiteCachePrefetchEntity form ){
    ...
    @ApiOperation("预取缓存删除")
    public R prefetchCacheDelete(@ApiIgnore @RequestAttribute("userId") Long userId,@RequestBody DeleteIdsForm form ){
